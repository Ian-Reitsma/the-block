...
use super::types::{ObjectManifest, ChunkRef, Redundancy, StoreReceipt};
use blake3::Hasher;
...
impl StoragePipeline {
    ...
    pub fn put_object(&mut self, data: &[u8], lane: &str) -> Result<StoreReceipt, String> {
        ...
        while offset < data.len() {
            ...
            self.db.insert(&format!("chunk/{}", hex::encode(id)), blob.clone());
            chunks.push(ChunkRef { id, nodes: vec!["local".into()] });
            offset = end;
        }
        ...
        self.db.insert(&format!("manifest/{}", hex::encode(man_hash)), manifest_bytes);
        let receipt = StoreReceipt { manifest_hash: man_hash, chunk_count: manifest.chunks.len() as u32, redundancy: Redundancy::None, lane: lane.to_string() };
        let rec_bytes = bincode::serialize(&receipt).map_err(|e| e.to_string())?;
        self.db.insert(&format!("receipt/{}", hex::encode(man_hash)), rec_bytes);
        Ok(receipt)
    }
    ...
    pub fn get_object(&self, manifest_hash: &[u8;32]) -> Result<Vec<u8>, String> {
        ...
        let manifest: ObjectManifest = bincode::deserialize(&manifest_bytes).map_err(|e| e.to_string())?;
        let key_bytes = manifest.content_key_enc.clone();
        let cipher = ChaCha20Poly1305::new(Key::from_slice(&key_bytes));
        let mut out = Vec::with_capacity(manifest.total_len as usize);
        for ch in manifest.chunks.iter() {
            let blob = self.db.get(&format!("chunk/{}", hex::encode(ch.id))).ok_or("missing chunk")?;
            if blob.len() < 12 { return Err("corrupt chunk".into()); }
            let (nonce_bytes, ct) = blob.split_at(12);
            let nonce = Nonce::from_slice(nonce_bytes);
            let plain = cipher.decrypt(nonce, ct).map_err(|_| "decrypt fail".to_string())?;

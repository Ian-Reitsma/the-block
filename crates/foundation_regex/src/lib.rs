//! Deterministic first-party regular expression engine.
//!
//! The implementation provides the subset of pattern syntax relied upon by
//! workspace tooling: anchors (`^`, `$`), character classes (with negation and
//! ranges), grouping, alternation, wildcards, and the repetition operators
//! (`*`, `+`, `?`, `{m,n}`).  Matching executes against UTF-8 scalar values so
//! behaviour is stable across platforms.

use std::collections::BTreeSet;
use std::error::Error;
use std::fmt;

/// Wrapper around the in-house regular expression engine.
#[derive(Clone, Debug)]
pub struct Regex {
    pattern: String,
    ast: Ast,
...
impl RegexError {
    ...
    /// Byte offset in the original pattern where the error occurred.
    ...
    /// The input pattern that failed to compile.
    pub fn pattern(&self) -> &str {
        &self.pattern
    }
}
...
fn match_repeat(
    ...
    while let Some((pos, min_left, max_left)) = stack.pop() {
        ...
        for next_pos in match_node(node, ctx, pos, match_start) {
            if next_pos == pos {
                // The inner node matched an empty string. Additional iterations would
                // loop forever, so terminate after ensuring the minimum requirement
                // is satisfied.
                if min_left == 0 {
                    results.insert(pos);
                }

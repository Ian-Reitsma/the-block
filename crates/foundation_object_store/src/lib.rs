...
use httpd::{BlockingClient, ClientError, HttpClient, Method};
use std::env;
...
impl S3Client {
    ...
    pub async fn put_object(
        ...
        self.put_object_at(client, bucket, key, body, now).await
    }
    ...
    pub async fn put_object_at(
        ...
        let prepared = self.prepare_put(bucket, key, &body, now)?;
        let mut request = client
            .request(Method::Put, &prepared.url)
    ...
    pub fn put_object_blocking(
        ...
        let now = UtcDateTime::from(SystemTime::now());
        self.put_object_blocking_at(client, bucket, key, body, now)
    }

    pub fn put_object_blocking_at(
        &self,
        client: &BlockingClient,
        bucket: &str,
        key: &str,
        ...
        let response = request.body(body).send().map_err(UploadError::Client)?;
        handle_response(
            response.status().as_u16(),
            response.into_body(),
            prepared.payload_sha256,
    ...
    fn prepare_put(
        ...
        body: &[u8],
        now: UtcDateTime,
    ) -> Result<PreparedRequest, UploadError> {
        signing_artifacts(&self.config, bucket, key, body, now)
    }
}
...
fn handle_response(
    ...
    if (200..300).contains(&status) {
        return Ok(UploadReceipt { payload_sha256 });
    }
...
mod tests {
    ...
    fn canonical_request_matches_known_example() {
        ...
        let prepared = signing_artifacts(&config, "examplebucket", "test.txt", body, now).unwrap();
        ...
        assert_eq!(
            ...
            "AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE/20130524/us-east-1/s3/aws4_request, SignedHeaders=content-length;host;x-amz-content-sha256;x-amz-date, Signature=de6cbb44503ba40c02823691744666556126f10a46ec5b7f3745eb44271bf1ee"
        );
        assert_eq!(
            prepared.payload_sha256,
            "44ce7dd67c959e0d3524ffac1771dfbba87d2b6b4b4e99e42034a8b803f8b072"

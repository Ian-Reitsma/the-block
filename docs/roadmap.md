# Status & Roadmap
> **Review (2025-09-25):** Updated readiness metrics, coding fallback rollout notes, and pillar percentages.

Mainnet readiness: 98.3/100 · Vision completion: 90.4/100.
The dependency-sovereignty pivot and wrapper rollout plan are central to every
...
timelines, wire SLA slashing dashboards atop the lane-aware matcher, finish the
remaining crypto dependency wrappers now that the coding crate fronts in-house
fallback algorithms, continue WAN-scale QUIC chaos drills with published
mitigation guides, extend bridge/DEX docs with multisig signer-set payloads plus
release-verifier walkthroughs, and finish the multisig wallet UX polish.
...
provider metadata to CLI/RPC consumers, and the RPC client keeps bounded retries
through clamped fault rates and saturated exponential backoff.

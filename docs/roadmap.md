# Status & Roadmap
> **Review (2025-10-11):** Hardened the `http_env` helper crate so every CLI/node/aggregator/explorer binary shares one TLS loader with diagnostics-backed warnings and observer hooks, shipped the `contract tls convert` and enhanced `contract tls stage` commands (canonical `--env-file` exports, service prefix overrides, PEM chain resilience, manifest generation with renewal reminders) for PEM→JSON conversion and asset fan-out, introduced the `tls-manifest-guard` CLI/systemd helper so reloads validate manifests, environment exports, and renewal windows before touching the binaries, migrated the remaining HTTP clients onto the new helpers, wired the aggregator to ingest node-side `tls_env_warning_total{prefix,code}` deltas while exposing the counter via dashboards and a `TlsEnvWarningBurst` alert, introduced integration tests that spin up the in-house HTTPS server to verify prefix selection, legacy fallbacks, converter round-trips, and the telemetry path, replaced the Python dashboard helper with the first-party monitoring snapshot binary while binary codec consolidation continues across node, crypto suite, telemetry, and harness tooling, and now persist the latest warning payloads at `/tls/warnings/latest` while `tls-manifest-guard --report` produces JSON summaries for orchestration pipelines. Subsequent hardening prunes `/tls/warnings/latest` snapshots after seven days, exercises the diagnostics→HTTP ingestion path end to end, and extends `tls-manifest-guard` with directory confinement, prefix enforcement, and env-file drift warnings.

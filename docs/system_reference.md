...
- In addition to kernel/benchmark metadata, `/wrappers` now emits `blocktorch_descriptor_digest` and `blocktorch_output_digest` (with the same labels as the receipts) so dashboards, CLI timelines, and governor status all report the encoded tensor descriptor and the deterministic inference output that produced the digest; if these values drift in dashboards, rerun `contract-cli compute stats --json` with `TB_BLOCKTORCH_DESCRIPTOR_DIGEST`/`TB_BLOCKTORCH_OUTPUT_DIGEST` overrides to confirm the metadata pipeline.
...
- Runtime execution now calls the BlockTorch runtime directly: inference and GPU-hash workloads run through the orchard `CpuContext` bridge (Metal when available, deterministic CPU otherwise) rather than a placeholder hash. The `kernel_variant_digest` now includes the Metal kernel bundle, build flags, and the runtime version stamp from `blocktorch/metal-tensor/runtime_version.txt`.
...
- Each inference slice still publishes an expected output hash inside `Job::slices`; compute matches must reproduce that hash to complete the proof loop. The `WorkloadRunner` now mixes the artifact + input through the orchard bridge (and records the runtime commit + proof latency in receipts) so identical metadata yields identical outputs.
...
- Receipts feed explorer/CLI timelines plus telemetry (`proof_verification_latency`, `orchard_alloc_free_delta`, `blocktorch_benchmark_commit`, `blocktorch_kernel_variant_digest`, `blocktorch_descriptor_digest`, `blocktorch_output_digest`) so operators can trace every job from submission through `tensor_profile_epoch` and every tensor descriptor/output pair to governor intent.
...
- Knobs (NodeConfig): `receipt_shard_count`, `receipt_blob_da_window_secs`, `receipt_header_activation_height`, `receipt_max_per_provider_per_shard`, `receipt_min_region_diversity`, `receipt_min_asn_diversity`. Defaults enable the header immediately (`receipt_header_activation_height=0`); tune the shard/DA/diversity knobs as needed.

...
- **RootAssembler pipeline:** `node/src/root_assembler.rs` replaces the temporary `BlobScheduler`. It ingests every submitted blob root together with its metadata (shard id, lane, size class, payload bytes, DA expiry, and retry hints) and turns them into deterministic `RootBundle` objects. Each `RootBundle` has a `bundle_slot` (a tick/slot index), a BLAKE3-based `bundle_hash`, a `size_class` tag (e.g., `L2` vs `L3`), and a vector of `MicroShardRoot` entries. Every entry records the root hash, originating shard, byte size, lane, and `available_until` deadline. Bundles are canonicalized by arrival order to keep the scheduling fully deterministic across miners and verifiers.
- **Scheduling and eviction:** `RootAssembler` maintains bounded queues per size class and enforces a strict cadence (`L2` bundles every 4 s, `L3` every 16 s) so the micro-shard root timeline is replayable even under load. When a queue fills, it drops the oldest entries (deterministically) and emits explicit telemetry so operators can tune `root_queue_depth` knobs. The assembler also tracks `bundle_slot` → `block_height` mappings so any node can reason about which bundle should be anchored by a given micro-block.
- **Anchoring rules:** Every micro-block header carries the `root_bundle_hash`, the `bundle_slot`, and the full `RootBundle` (hashlayout includes the bundle payloads). Macro-block checkpoints contain a compact `RootCheckpoint` (one per size class) summarizing the span of anchored bundles since the last macro-block. By anchoring the bundle hash into the consensus hash layout, we make the notarization protocol consensus-critical.
- **Validation rules:** During block import, `node/src/lib.rs` recomputes the expected slot for each bundle using the consensus timestamp, enforces monotonic slot progression per size class, and validates the bundle hash. Blocks that reference out-of-order bundles are rejected. Replay tests (see `node/tests/root_bundle_replay.rs`) recompute the `RootAssembler` timeline to guarantee determinism.
- **Availability and retries:** The assembler marks every bundle with an `available_until` deadline matching the blob DA window; verifiers ensure each `MicroShardRoot` entry remains fetchable (via `MicroShardAvailabilityTracker`) before including a new bundle. If DA sources drop data before the deadline, nodes retry the bundle in the next slot and emit `root_replay_retry_total` telemetry so the governor can detect DA instability.
- **Light-client consumption:** The RPC surface `microshard.roots.last` reports a lightweight view of the latest `RootBundleSummary` (`bundle_slot`, `bundle_hash`, `ready_root_count`, `size_class`, `available_until`). Light clients can anchor their state by validating the bundle hash against the block header without replaying the blobs themselves.

This pipeline keeps the Block Vision's “micro-shard roots per tick” promise and is now consensus-critical: bundles feed hashlayout, validation, and replay invariants instead of the former placeholder scheduler.
...
- Mesh deliveries now emit deterministic **Relay receipts** that anchor the carry-to-earn economic loop, enforce per-ack TTLs/budgets, and flow through the same telemetry/governor wiring as the other markets.

#### Relay & Carry Economics
- **Job lifecycle:** Each mesh-enabled impression becomes a job offer (payload + campaign metadata). Range Boost claims the job by enqueuing the payload, records hop proofs as it traverses the mesh, and when a bundle successfully exits the queue a relay receipt is minted, anchored to the serving block, and routed through the settlement pipelines so the economics controller can replay bytes, prices, and mesh signal histories deterministically.
- **Receipt shape:** `RelayReceipt { job_id, mesh_payload_digest, bytes, price_per_mib_usd_micros, total_usd_micros, clearing_price_usd_micros, resource_floor_usd_micros, campaign_id?, creative_id?, mesh_peer?, transport?, latency_ms?, hop_proofs, delivered_at_micros, block_height, provider }` mirrors the existing market receipts so ledger replay, telemetry, and gating stay unified. Jobs share a hashed `job_id` derived from the signed `ReadAck` + selection metadata, guaranteeing a single canonical receipt per mesh delivery.
- **Resource guardrails:** Job offers are validated against `TB_RELAY_MAX_BYTES_PER_BUNDLE` (default 2 MiB), `TB_RELAY_MAX_BYTES_PER_EPOCH` (default 64 MiB), and `TB_RELAY_MAX_ACK_AGE_SECS` (default 60 s) before they ever touch the queue. Oversized, stale, or budget-exhausted offers are dropped with telemetry (`relay_job_rejected_total{reason}`) so operators can trace guards without risking runaway drains.
- **Shadow → Trade gating:** Relay receipts now inherit the storage market gate: when `launch_storage_autopilot` is in rehearsal, Range Boost stays in telemetry-only “shadow” mode and drops receipts after validation. Once the governor flips storage into trade, relay receipts are minted and routed into blocks. Local operators can still pin `TB_RELAY_ECONOMICS_MODE=shadow` to force dry-runs even after the governor opens the gate, but they can no longer mint receipts while storage is gated shut.
...
- The DHT-backed storage marketplace blends on-chain contracts with the overlay-level provider graph. `storage_market::StorageMarket` keeps a second tree for `market/providers` so discovery can run without a centralized catalog, `node/src/storage/marketplace.rs` translates gateway queries into `storage_market::DiscoveryRequest`, and `node/src/storage/pipeline.rs` surfaces `StoragePipeline::marketplace_search_options` to bias provider picks by recent quota snapshots and region hints.
- Provider profiles are signed advertisements: `node/src/storage/provider_directory.rs` wraps the marketplace and publishes `ProviderProfile` records (versioned, with `expires_at` TTLs) over the gossip overlay as `Payload::StorageProviderAdvertisement` messages. Every advertisement is signed by the node’s network key, carries a monotonic `version`, and defaults to a 15‑minute TTL; stale/older profiles are rejected and counted in telemetry. `TB_STORAGE_MARKET_DIR` remains a cache only—the overlay is the source of truth, and `StorageMarket::discover_providers` merges network candidates with the cache before sorting.
- Provider selection follows three phases:
 1. The gateway/CLI populates `storage_market::ProviderProfile` via `storage.register_provider` (see `node/src/rpc/storage.rs`), which persists capacity, price, deposit, latency, and tag metadata plus proof counters.
 2. Uploads call `storage.discover_providers` to run the DHT query (object size, shares, optional region, SLAs) and receive the cheapest list sorted by price, freshness, and success rate; `storage_market::DiscoveryRequest` enforces minimum chunk capacity and success-rate guardrails before returning up to `limit` providers, and `node/src/storage/pipeline.rs` exposes an opinionated `SearchOptions` template so placement logic never drifts from the marketplace schema.
 3. Contracts target the selected providers: `node/src/rpc/storage.rs::upload` builds `StorageContract`/`ReplicaIncentive` entries driven by the pricing distribution, escrow rates, and retention terms captured in the DHT catalog.
- Discovery now uses a request/response overlay protocol: `StorageProviderLookup` messages (signed, rate-limited, replay-protected) hop across peers, responses hydrate the persisted provider cache (`storage_market::StorageMarket::cache_provider_profile`), and lookups survive restarts because the directory preloads profiles from disk on install.
- Serialization and schema drift are guarded by `storage_market::codec::{serialize_provider_profile, deserialize_provider_profile}` tests so `contract-cli storage register-provider`, RPC `storage.register_provider`, and the SimpleDb snapshots all share the same JSON schema for provider metadata plus proof counters.
- Retrieval proofs still run through `node/src/rpc/storage.rs::challenge`, but each proof now updates `storage_market::StorageMarket::record_provider_feedback` so providers' success/failure counters stay DHT-consistent and later discovery requests automatically prefer reliable peers. Settlements emit `storage_market::receipts::StorageSettlementReceipt` objects that gateways drain via `node/src/rpc/storage.rs::drain_storage_receipts` so the ledger can file `StorageReceipt` entries without caring whether the proof originated from a repair loop or a reactive download.
...
**BlockTorch integration**: The `blocktorch/` stack (metal-tensor + autograd) provides the deterministic tensor layer for ML workloads executed through the compute marketplace. BlockTorch defines the kernel set, gradient serialization, and proof-ready metadata needed for SNARK attestation and pricing via `ORCHARD_TENSOR_PROFILE`. The strategic roadmap, telemetry wiring, and governor coordination workflow live in [`docs/ECONOMIC_PHILOSOPHY_AND_GOVERNANCE_ANALYSIS.md`](ECONOMIC_PHILOSOPHY_AND_GOVERNANCE_ANALYSIS.md#part-xii-blocktorch--the-compute-framework-strategy) and [`docs/blocktorch_compute_integration_plan.md`](docs/blocktorch_compute_integration_plan.md). Execution priority and backlog responsibility remain recorded in `AGENTS.md §15.B.1`, while `docs/operations.md#telemetry-wiring` now documents the exact metrics (`proof_verification_latency`, `orchard_alloc_free_delta`, `snark_prover_latency_seconds`) operators should monitor before approving upgrades.
...
- **Relayer incentives and survivability:** Rewards accrue per completed duty (deposit, withdrawal, settlement) once proofs pass and the duty completes before its `bridge_duty_window_secs` deadline. Pending duties past that window are auto-marked expired, slashed by `bridge_failure_slash`, and surfaced in `bridge.dispute_audit` to keep operators in sync. Capacity is bond-backed: each relayer may only carry `max(1, bond / bridge_min_bond)` pending duties; attempts beyond that return `bridge_capacity_exhausted` so spam cannot starve the system without additional stake.
- **Claim/anti-griefing rules:** Reward claims remain RPC/CLI-driven but are blocked while any duty for that relayer is pending or expired; the node forces a sweep of expired work before honoring claims. Claims still require governance approvals, debit the pending rewards gauge, and emit `bridge_reward_claims_total`/`bridge_reward_approvals_consumed_total` plus receipts (`bridge.reward_claims`) so operators can audit who claimed, when, and against which approval key. Capacity/expiry telemetry feeds the bridge row on the dashboard so “bridge health” is a single screen.
...
|----------------|-------|-------|
| **economics** | Block reward + subsidy autopilot | Shadow mode tracks `NetworkIssuanceController` outputs (per-block rewards derived from count/volume/utilization with only the logistic miner-fairness multiplier applied), the persisted `economics_block_reward_per_block`, and `economics_prev_market_metrics` gauges. The governor samples the current chain with `economics::replay::replay_economics_to_tip`, which replays the receipts via `economics::deterministic_metrics::derive_market_metrics_from_chain` to recompute `economics_epoch_*`, treasury inflow, and the same reward/hash snapshot that flows through `/wrappers`. The gate flips only after those replay-derived values stay within bounds for a streak and match `/wrappers` telemetry. |

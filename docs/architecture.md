...
- **Anchoring rules:** Every micro-block header now carries the `root_bundle_hash`, the `bundle_slot`, and either the full `RootBundle` or an index into the shared append-only `root_manifest` (see `node/src/storage/root_manifest.rs`). Macro-block checkpoints contain a compact `RootCheckpoint` (one per size class) summarizing the span of anchored bundles since the last macro-block. By anchoring the bundle hash into the consensus hash layout, we make the notarization protocol consensus-critical.
- **Validation rules:** During block import, `node/src/blockchain/validation.rs` recomputes the expected `RootBundle` for the current `bundle_slot` using the persisted `RootAssembler` queue and validates that the bundle hash in the header matches the recomputed hash. Blocks that reference out-of-order or expired bundles are rejected; missing `MicroShardRoot` metadata causes `BlockValidationError::RootScheduleMismatch`. Replay tests (see `tests/root_bundle_replay.rs`) recompute the entire `RootAssembler` timeline across long chains to guarantee determinism.
...
- **Light-client consumption:** The RPC surface `microshard.roots.last` now reports a lightweight view of the latest `RootBundleSummary` (`bundle_slot`, `bundle_hash`, `ready_root_count`, `size_class`, `available_until`). Light clients can anchor their state by validating the bundle hash against the block header without replaying the blobs themselves.

This spec rewrites the placeholder blob chain into a first-class notarization protocol that keeps the Block Vision's “micro-shard roots per tick” promise. Future code will implement the `RootAssembler`, persist bundles in the manifest, wire the new header fields through `hashlayout`, and ensure replay/resiliency guarantees described above before macro-validator and light-client stacks rely on the new anchors.
...
- **Shadow → Trade gating:** `TB_RELAY_ECONOMICS_MODE` controls whether relay receipts seed blocks (`trade`) or remain in the telemetry-only “shadow” mode until operators/Launch Governor signal readiness. Once the governor gate flips into `trade`, receipts start influencing the economic snapshot and payout logs; until then the mesh emits the same `relay_*` counters for validation without minting payouts.

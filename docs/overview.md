...
| `blocktorch/` | BlockTorch tensor runtime (metal-tensor + autograd), profiling hooks, and the PyTorch bridge used for validation. |
...
**Technical Details:**
- **Code surface** — `crates/energy-market` implements providers, credits, receipts, and telemetry; `node/src/energy.rs` persists them in sled (`SimpleDb::open_named(names::ENERGY_MARKET, …)`), applies governance hooks, and exposes health checks. RPC handlers live in `node/src/rpc/energy.rs`, the CLI entry point is `cli/src/energy.rs`, and oracle ingestion goes through `crates/oracle-adapter` plus the `services/mock-energy-oracle` binary used by the Block OS drill.
...
| **BlockTorch** | Deterministic tensor/autograd framework (blocktorch) for verified compute workloads in the marketplace. | [`ECONOMIC_PHILOSOPHY_AND_GOVERNANCE_ANALYSIS.md`](ECONOMIC_PHILOSOPHY_AND_GOVERNANCE_ANALYSIS.md#part-xii-blocktorch--the-compute-framework-strategy) |
...
- **Dependency review gate** — Reviewers must reject any registry/git dependency. Keep `FIRST_PARTY_ONLY=1`, require `dependency_guard` to pass, and ensure `config/dependency_policies.toml` + `provenance.json`/`checksums.txt` are updated whenever vendoring changes. PR descriptions must call out the zero third-party policy explicitly for dependency touches.
...
- **BlockTorch compute framework** — The blocktorch stack and coordinator plan follow the Part XII checklist in `docs/ECONOMIC_PHILOSOPHY_AND_GOVERNANCE_ANALYSIS.md` and the priority ordering in `AGENTS.md §15.B.1`. `tb-cli compute stats` and `tb-cli governor status` now publish the BlockTorch job timeline (kernel digest, benchmark commit, proof latency, aggregator trace hash) and respect `TB_BLOCKTORCH_KERNEL_VARIANT_DIGEST` / `TB_BLOCKTORCH_BENCHMARK_COMMIT`, so operators can tie governor decisions to the same telemetry bucket.

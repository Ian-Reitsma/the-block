...
- **Auditing workflow**
  ...
  5. The governor now verifies that the persisted snapshot hash still matches the intent payload before mutating runtime params; any mismatch leaves the intent pending and logs “governor intent snapshot verification failed; delaying apply” so you can fix the artifact before allowing the gate to apply.

  6. Telemetry derived from `replay_economics_to_tip` is now panic-safe: if deterministic replay ever fails (log line `governor_economics_replay_panic`), the governor surfaces the stored epoch counters (`economics_epoch_*`, `economics_prev_market_metrics`) as a fallback, keeping the `/wrappers` hash and dashboards alive until the underlying issue is resolved.
...
- **Dashboards and wrappers**
  ...
  - `/wrappers` also surfaces the storage adoption plan metrics (`storage_adoption_plan_coverage_percent`, `storage_adoption_plan_cost_per_share`, `storage_adoption_plan_estimated_total_cost`, `storage_adoption_plan_selected_provider_count`, `storage_adoption_plan_required_provider_count`) so auditors can compare the CLI `contract-cli storage adoption-plan` output with the latest checksum. Regenerate both `/wrappers` snapshots, capture the new hash, and refresh any Grafana rows that visualize these gauges whenever the plan wiring evolves.

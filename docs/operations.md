...
- **Ad market observability contract** — Ad readiness must be provable from Grafana. Whenever selectors, privacy budgets, presence proofs, or readiness reports change:
  ...
  - Record the `/wrappers` hash emitted by `tb-cli telemetry wrappers` for the ad-market namespace and include it in the PR description along with the metrics command transcripts.
...
- Compute-market panels now include SLA slashing + remediation widgets fed by `compute_sla_slash_total{lane}` and `match_loop_latency_seconds{lane}`. Pair them with `tb-cli compute lanes --format json` for direct remediation workflows per `AGENTS.md §15.B`.
- **Ad market operations** — Treat selector/presence changes like release events:
  1. **Snapshot readiness** — Run `tb-cli ad-market readiness --with-privacy --format json` before and after changes. Archive the `/wrappers` hash, JSON payload, and Grafana “Ad Market Readiness”/“Presence Freshness”/“Selector Competitiveness” panels in the PR or incident log.
  2. **Inventory sanity** — Use `tb-cli ad-market inventory --selector domain_tier=premium --selector presence_bucket=<bucket>` and `tb-cli ad-market list-campaigns --selector interest_tag=travel` to confirm new selectors appear in cohort prices. Record CLI output in the worklog.
  3. **Presence management** — Operators mint LocalNet/Range Boost receipts via their respective CLIs, then monitor `ad_market.list_presence_cohorts` for stale buckets. `tb-cli ad-market presence list --min-confidence 9500` enumerates buckets; `tb-cli ad-market presence reserve --campaign <id> --bucket <bucket-id> --slots N` proves the reservation flow still honors governance TTL/radius knobs.

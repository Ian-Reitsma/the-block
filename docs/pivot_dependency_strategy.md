...
> **Review (2025-10-10):** Completed the node/crypto suite migration off the legacy bincode profiles, rewired telemetry to the named binary profiles, and moved the simulation harness/CLI defaults onto the first-party codec facade while documenting the tooling crates still carrying serde/json fallbacks.
> Dependency pivot status: Runtime, transport, overlay, storage_engine, coding, crypto_suite, codec, and serialization facades are live with governance overrides enforced (2025-10-10); node telemetry and harness automation now emit binary profile labels instead of bincode.
...
- ✅ Installer packaging now emits `.tar.gz` bundles through `foundation_archive`'s streaming TAR/Gzip APIs, eliminating the legacy `zip` dependency from release tooling.
- ✅ `foundation_archive::gzip` exposes streaming encoders/decoders so large support bundles can be compressed or inspected without loading entire blobs into memory; fixtures cover interoperability with legacy stored-block gzip payloads.
- ✅ Gateway fuzzing now exercises the first-party HTTP parser (`gateway::http::parse_request`), enabling the removal of `httparse` from the workspace.
- ✅ Rate-limit filters consume the in-house `concurrency::filters::xor8::Xor8` backend, eliminating the `xorfilter-rs` dependency and keeping telemetry consistent.
- ✅ Simulation, fuzzing, and storage repair harnesses route binary payloads through `foundation_serialization::binary`, clearing the last direct `bincode` calls outside production crates.

...
- `ad_market.record_conversion` now enforces advertiser authentication entirely
  ...
  <account>:<token>`, and the handler rejects missing headers, mismatched
  accounts, missing hashes, or invalid tokens with explicit error codes before
  recording uplift snapshots—all without introducing serde/json fallbacks.
- `ad_market` RPCs now surface geo/device/CRM/delivery selectors, uplift holdout
  assignments, and mesh payload metadata strictly through the existing
  `foundation_serialization::json` builders. Gateway plumbing enriches
  `ImpressionContext`, `SelectionReceipt`, and `ReadAck` using the same helpers,
  RangeBoost mesh requests enqueue payloads via the first-party queue, and the
  new `ad_market.record_conversion` method persists uplift snapshots without
  introducing serde/json fallbacks. Genesis and block RPC responses reflect the
  expanded ad-settlement footprint (dual-token IT payouts, treasury events,
  delivery metadata) with compile-time verification still anchored on
  `hash_genesis::calculate_genesis_hash`.
- `ad_market.reserve_impression` receipts now embed verifier-committee metadata,
  stake snapshots, and transcripts generated by the new first-party
  `verifier_selection` crate. VRF outputs/proofs ride the in-tree
...
- Budget shading guidance now reports requested κ, applied multipliers, shadow
  ...
  trace without introducing optimisation or serde dependencies.

...
- Wallet command `Wallet::discover` (exposed via `wallet discover-signers --timeout <ms>`) performs a UDP broadcast and lists the HTTP endpoints that answered; add `--json` for automation scripts so they can read `{"timeout_ms":<ms>,"signers":["http://..."]}` without scraping human prompts. Each discovery run tallies `remote_signer_discovery_total`, and successful responses increment `remote_signer_discovery_success_total`, so operators can compare `/wrappers` summaries with the remote signer fleet’s availability history.
- `crates/wallet/tests/remote_signer.rs::remote_signer_discover_replies` now reproduces the `theblock:signer?` handshake inside the test harness, keeping discovery deterministic and giving us a regression test that the CLI emits the promised JSON contract before we wire dashboards or automation tooling against it.
- `/wrappers` and the `monitoring/grafana/telemetry.json` Remote Signers row now expose the discovery counters (attempts, successes, and success ratio) so operators can correlate CLI probes, `/wrappers` snapshots, and dashboards while alerting on failure modes without guessing which metric to chart.
...
- **Dispute hooks** — Until dedicated dispute RPCs ship, governance proposals (e.g., raising `energy_slashing_rate_bps`, pausing settlement) are the primary kill switch. Record suspect `meter_hash` values via `contract-cli energy market --verbose`, attach them to proposals, and document rollback steps. The new `energy.slashes` RPC/`contract-cli energy slashes` command and `/governance/energy/slashes` explorer view now publish ledger-backed slash receipts for quorum, expiry, or conflict rejections, and the accompanying telemetry counters (`energy_quorum_shortfall_total`, `energy_reading_reject_total{reason}`, `energy_dispute_total{state}`) expose the failure reason so operators can triage before applying rollbacks or slashes.
- **Mock oracle isolation** — `services/mock-energy-oracle` is a dev/testnet binary only. It uses mock signatures (provider_id||kwh) and intentionally relaxed auth. Never expose it to production networks; wrap it in loopback-only listeners when exercising `scripts/deploy-block_os-testnet.sh`.

...
delivery, `mesh_payload_digest` exposes the BLAKE3 digest of any staged mesh
payload, and `mesh_payload_bytes` records the payload length. Auditors should
compare these values with the reservation logs and diagnostic events (e.g.
`range_boost_forward_*`) to ensure mesh deliveries were properly staged, holdout
and treatment cohorts received identical payloads, and that HTTP impressions did
not carry mesh content unexpectedly.

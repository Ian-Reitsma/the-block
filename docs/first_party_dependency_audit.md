...
| light_client | `crates/light-client/src/state_stream.rs` | 63-180, 204-220, 748-826 | manual `Serialize`/`Deserialize` + binary fixture | Cached account/persisted state structs implement first-party serialization with a matching reference encoder; fixture coverage now toggles FIRST_PARTY_ONLY between `1`, `0`, and unset to guarantee guarded and unguarded builds share the same bytes. |

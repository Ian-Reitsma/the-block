...
|-------------|----------------|---------------------|------------------|
| **Blob chain root scheduler in `node/src/blob_chain.rs`** | [`docs/architecture.md#ledger-and-consensus`](docs/architecture.md#ledger-and-consensus) + [`docs/block_os-spec/01-core-l1.md#blob-root-scheduler`](docs/block_os-spec/01-core-l1.md#blob-root-scheduler) now point at the implemented RootAssembler pipeline (`node/src/root_assembler.rs` and the `blob_chain` shim). | ✅ Delivered: RootAssembler emits deterministic `RootBundle`s, is hashed into block headers, validated for slot monotonicity on import, and replayed in `node/tests/root_bundle_replay.rs`. | `cargo test -p the_block --test root_bundle_replay` (determinism) plus `cargo test -p the_block --test replay` for full-chain replays. |

...
> cross-checks committee weight ppm values, gateway tests assert shading telemetry
> for every candidate in multi-creative traces, and integration coverage drives
> stale snapshots, transcript mismatches, and weight inflation through
> `ad_market.reserve_impression` to prove invalid committees are stripped before
> settlements land. Benchmarks now publish straight into the monitoring stack
> under a shared lock: setting `TB_BENCH_PROM_PATH` writes
> `benchmark_ann_soft_intent_verification_seconds` plus `_p50`/`_p90`/`_p99`

...
- `ad_budget_config_value{parameter}` captures live pacing parameters
  (`epoch_impressions`, `step_size`, `dual_step`, `dual_forgetting`, `max_kappa`,
  `min_kappa`, `shadow_price_cap`, `smoothing`, `epochs_per_budget`).
  ...
  headroom, shadow price, and epoch target. `ad_budget_shadow_price{campaign}`
  mirrors the dual variable updated by the primal–dual loop; spikes register via
  `ad_budget_shadow_price_spike_total`. Cohort metrics—
  `ad_budget_cohort_kappa{campaign,domain,provider,badges}`,
  `ad_budget_cohort_error{...}`, `ad_budget_kappa_gradient{...}`, and
  `ad_budget_cohort_realized_usd{...}`—track pacing multipliers, gradient
  pressure, and realised spend for each cohort. The telemetry module removes
  stale label values when snapshots change, so the gauges remain first-party
  without leaking historical labels. `ad_budget_summary_value{metric}` aggregates
  snapshot analytics (`campaign_count`, `cohort_count`, `mean_kappa`,
  `max_kappa`, `mean_smoothed_error`, `max_abs_smoothed_error`,
  `realized_spend_total`, `epoch_target_total`, `epoch_spend_total`,
  `dual_price_max`, `config_step_size`, `config_dual_step`,
  `config_dual_forgetting`, `config_max_kappa`, `config_min_kappa`,
  `config_shadow_price_cap`, `config_smoothing`) so dashboards can alert on
  pacing drift without rebuilding summaries.
  `ad_budget_snapshot_generated_at_micros` marks the most recent snapshot
  timestamp so dashboards can alert when pacing exports stall and `ad_budget_progress`
  remains the top-level pacing gauge.
  - `ad_resource_floor_component_usd{component,domain,provider}` decomposes the
    composite resource floor enforced per impression. `component="bandwidth"`
    captures bytes × price-per-MiB, `component="verifier"` reports the amortised
    proof cost using the cohort-specific qualified impression estimate, and
    `component="host"` surfaces the venue fee. The `component="total"` series
    equals the clearing floor recorded in receipts.
  - `ad_resource_floor_impressions_per_proof{domain,provider}` publishes the
    per-cohort `q_c` estimate used to amortise verifier costs inside the composite
    floor so operators can detect anomalous cohort sizes.
  - `ad_privacy_budget_total{family,result}` and
    `ad_privacy_budget_remaining{family,metric}` expose the badge-family privacy
    budget ledger. Accepted, cooling, and revoked counters let dashboards spot
    abuse, while the `metric` gauge pair (`epsilon`/`delta`) tracks the remaining
    differential-privacy allowance per family after decay is applied.
  - `ad_uplift_propensity{sample}` gauges the per-bucket propensity estimate while
    `ad_uplift_lift_ppm{impressions}` records the cross-fitted doubly-robust lift
    histogram so uplift regressions and calibration drift remain observable without
    replaying training logs.
...
- `tls_env_warning_total{prefix,code}` and
  ...
  `tls_env_warning_variables_fingerprint{prefix,code}`) expose BLAKE3 hashes of
  the most recent warning detail string and variable payload so dashboards can
  detect drift without storing free-form text, and now emit integer-coded
  values so the full 64-bit digest survives ingestion without IEEE754 rounding.
  Companion counters
  ...
  accumulate per-fingerprint event totals to show which hashed payloads are
  recurring. `tls_env_warning_detail_unique_fingerprints{prefix,code}` and
  `tls_env_warning_variables_unique_fingerprints{prefix,code}` report how many
  distinct hashes have ever been observed for a given prefix/code, and the
  aggregator emits an `observed new tls env warning … fingerprint` info log
  whenever a non-`none` fingerprint shows up for the first time. The aggregator
  also exports
  ...
  `tls_env_warning_least_recent_last_seen_seconds` for retention health. Nodes
  and the aggregator now increment
  `tls_env_warning_events_total{prefix,code,origin}` via the shared
  diagnostics sink so dashboards can distinguish warnings emitted by
  on-host diagnostics observers (`origin="diagnostics"`) from peer-ingest
  ...
  and per-fingerprint occurrence counts. The CLI exposes
  `contract telemetry tls-warnings` (with `--json`, `--prefix`, and `--code`
  filters plus `--probe-detail`/`--probe-variables` for local fingerprint
  calculations) so operators can compare local and aggregated views without
  scraping `/metrics`. Text output now prints per-fingerprint tallies alongside
  the hex fingerprints (e.g. `fingerprint_counts: detail=0123abcd=4 vars=none=1`)
  and carries an `ORIGIN` column that mirrors the Prometheus
  `tls_env_warning_events_total{prefix,code,origin}` label so on-host snapshots
  match dashboard pivots.
  to mirror the Prometheus counters. The autogenerated Grafana dashboards add
  dedicated TLS fingerprint panels: the hash gauges show the latest
  `tls_env_warning_*_fingerprint` values per prefix/code, the unique panels track
  `tls_env_warning_*_unique_fingerprints`, and the delta panels aggregate
  `increase(tls_env_warning_*_fingerprint_total[5m])` so an ops rotation can spot
  surges without querying Prometheus directly. The `monitoring` crate now ships a
  `compare-tls-warnings` binary that consumes
  `contract telemetry tls-warnings --json`, fetches `/tls/warnings/latest`, and
  verifies the `tls_env_warning_*` series from `/metrics`; any drift emits a
  mismatch summary and a non-zero exit code to anchor incident automation.

Aggregator support bundles produced via `/export/all` include
`tls_warnings/latest.json` and `tls_warnings/status.json` entries so offline
analysis retains the hashed detail/variable payloads, per-fingerprint counts,
and retention metadata alongside the peer metric history.

...
> **Reference:** The canonical formulas live in `docs/economics_and_governance.md` and `docs/architecture.md`. This progress log summarizes what has landed in code so far; if implementation and documentation ever disagree, update the docs first (per `AGENTS.md §0.2a`) and treat the diff as a bug.

...
> **Source of truth:** The live spec for each formula now resides in `docs/economics_and_governance.md` (monetary policy, subsidies, multipliers, tariffs) and `docs/architecture.md` (launch-governor gates, networking controls). This plan is the rollout tracker; if code diverges from the docs, treat it as a bug and sync the spec before editing implementations.

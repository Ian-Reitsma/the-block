...
`mempool/scoring.rs` contains the reputation-weighted scoring model. The helper `FeeFloor::new(size, percentile)` now requires explicit percentile inputs so tests, simulations, and CLI helpers always operate with the same governance-selected quantile. The current floor is exported as the `fee_floor_current` gauge for telemetry and explorer visualisation.

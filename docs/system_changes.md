...
## 2024 Third-Token Ledger Removal and CT Subsidy Transition

### Background: Legacy Three-Token Model
For the first development iterations, The‑Block experimented with a tri-token economy. Consumptive actions such as storing blobs or serving HTTP reads were paid with non-transferable third-token units that decayed over time, while CT and IT remained liquid utility tokens. These units were earned via uptime, storage proofs, or domain budgets, and providers had to periodically swap or burn them to realize value. Documentation and code referenced a `read_reward_pool` that minted these units to reimburse gateways for free user reads.

### Rationale for the Switch
- **Operational Friction:** Operators juggled third-token balances, decay curves, and swap mechanics. New node deployments stalled when those balances ran out.
...
- **Simpler UX:** The project vision prioritizes "wallet opens, everything works". Requiring a third token contradicted that mandate.

After multiple testnet cycles and stakeholder discussions, governance approved a migration to a pure CT/IT model where inflation-funded subsidies replace the third-token ledger.

### Implementation Summary
- Deleted the entire third-token module, its associated RPC endpoints, and the legacy ledger database.
...
- Redirected all former third-token slashing paths to explicit CT burns, ensuring punitive actions reduce circulating supply.

Each change shipped behind feature flags during the transition period and was
accompanied by migration scripts (such as `scripts/purge_legacy_ledger.sh` and new genesis
...
### Impact on Operators
- Rewards arrive entirely in liquid CT, removing the need for off-chain swaps of a third token.
...
rent-escrow metrics; a missing gauge usually indicates lingering legacy ledger
config files or dashboard panels.

### Impact on Users
- Uploads, hosting, and dynamic requests no longer require preloading a third token. Wallet creation suffices to start using storage or web hosting features.
...
Wallet interfaces now omit any third-token balance widgets. When a user uploads a
...
### Pros and Cons
| Aspect | Third-Token Ledger | CT Subsidy Model |
|-------|---------------------|------------------|
| Operator payouts | Non-transferable third-token units | Liquid CT every block |
| UX for new users | Requires earning or buying a third token | Wallet works immediately |
| Governance surface | Third-token mint/decay curves | Simple multiplier votes |
...
### Migration Notes
Devnet operators should run `scripts/purge_legacy_ledger.sh` to wipe obsolete ledgers and regenerate genesis files without the legacy balance field. Faucet scripts now dispense CT. Operators must verify `inflation.params` after upgrade and ensure no `read_reward_pool` references persist in configs or dashboards.

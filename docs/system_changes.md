...
> **Review (2025-10-29, dawn):** Selection receipts now enforce the truthful floor
> under a manifest registry, and pacing knobs surface controller health. The
> embedded manifest still ships with the binary but is installed into a runtime
> registry guarded by `foundation_lazy::Lazy`; `install_selection_manifest`
> hand-parses JSON, rejects revision regressions, and only advances the manifest
> epoch when the descriptor set is monotonic so wallets cannot downgrade circuits.
> `selection_manifest_version`, `selection_manifest_tag`, and
> `selection_circuit_summaries()` expose the active descriptors, while
> `SelectionProofMetadata` threads manifest epoch/tag, transcript domain separator,
> expected witness commitments, and committee transcript digests into every
> receipt so settlements prove both welfare clearing and committee co-signature.
> Rejections still bubble up through
> `ad_selection_attestation_total{reason=semantics|revision|format|digest}` for
> dashboards. Pacing parameters—PI gains, forgetting factor, target utilisation,
> smoothing—remain exported via RPC (`ad_market.inventory`, `ad_market.budget`,
> `ad_market.list_campaigns`) and now ship a `status`/`reason` pair that flags
> controller bounds (ηₚ, ηᵢ, forgetting) using the same first-party JSON helpers.
> The synthetic load scenario (`node/tests/ad_market_synthetic.rs`) drives
> manifest updates, SNARK attestations (including committee transcripts), pacing
> status, and broker rollovers under load so failure paths stay covered without
...
> through `ad_market.inventory`/`distribution` RPCs and the new CLI subcommand.
> Gateway matching consumes refreshed provider badge metadata so badge-targeted
> campaigns only serve providers with the required physical presence. Explorer
> payouts pick up tabular and Prometheus rendering, the metrics aggregator seeds
> role counters at startup, and new `explorer_block_payout_{read,ad}_last_seen_timestamp`
> gauges backstop dashboards and alerts whenever payout streams stall.
> **Review (2025-10-31, late evening):** Bridge remediation tests extend the `RemediationSpoolSandbox` to cover page/throttle/quarantine/escalate directories and add `remediation_spool_sandbox_restores_environment` so the `TB_REMEDIATION_*_DIRS` guards are proven to unwind without residue. Explorer payout ingestion layers `explorer_payout_counters_are_peer_scoped` onto the churn regression, demonstrating per-peer caches ignore regressions even when explorers report disjoint role totals—all with trace-only diagnostics and no negative deltas.
> **Review (2025-10-31, afternoon):** Bridge remediation tests now provision a dedicated `RemediationSpoolSandbox` per scenario so spool hooks land inside ephemeral directories with automatic teardown, removing the `/tmp` hygiene burden while keeping `TB_REMEDIATION_*_DIRS` scoped to each run. Explorer payout ingestion clamps counter regressions to the last observed high, emits trace diagnostics instead of negative deltas, and ships a churn-focused regression that alternates read/advertising role sets to guarantee caches remain monotonic when peer labels rotate between scrapes.
> **Review (2025-10-24, early afternoon):** Explorer ingestion now verifies binary-vs-JSON payout decoding in integration tests, while the metrics aggregator rewrites the payout recorder to increment `explorer_block_payout_*_total` counters directly from cached handles so Prometheus/Grafana panels see live deltas. Monitoring documentation now captures the cache semantics and automation snippets show operators how to query payouts by hash or height without leaving first-party tooling.
...
> can audit per-target delivery without scraping metrics. Operator runbooks link
> directly to the generated steps, and the alert validator picked up
> dispute/quorum recovery fixtures so the bridge skew heuristics stay quiet while
> anomalies cool off.
...
> existing action counter, with CLI tests covering spool successes, failures,
> and skipped hooks. Grafana/GHTML dashboards gained a dispatch health panel so
> operators can track HTTP/spool outcomes in real time, and the incident playbook
> points responders at the new legend entries for rapid diagnostics.
> **Review (2025-10-24, midday):** Bridge remediation actions now dispatch
> directly to first-party paging/governance hooks via the new
> `TB_REMEDIATION_*_URLS`/`*_DIRS` environment variables. The aggregator logs
> HTTP success/failure, persists spool payloads with deterministic filenames, and
> the operator incident playbook documents the liquidity response sequence that
> ties dashboard annotations to the dispatched playbook. The shared alert
> validator picked up recovery-curve and partial-window fixtures so the
> `BridgeCounter*Skew` heuristics stay stable across edits.
...
> **Review (2025-11-01, early morning):** Bridge CLI regressions now assert that optional dispute-audit filters stay well defined. `bridge_dispute_audit_serializes_optional_fields` feeds `asset=None`/`cursor=None` through `handle_with_transport` and confirms the JSON payload emits explicit `null` entries while retaining the default 50-row page size. A companion parser regression, `bridge_dispute_audit_parser_defaults_limit_and_cursor`, runs the command builder via the first-party `Parser` so the fallback URL and limit remain guarded without relying on clap-derived code. Monitoring gained `dashboards_include_bridge_counter_panels`, parsing every generated Grafana JSON (dashboard/operator/telemetry/dev) to ensure the reward-claim, approval, settlement, and dispute panels keep their intended expressions and legends across templates. Together these tests keep governance audit tooling and dashboards hermetic without reintroducing legacy transports or third-party validators.

...
> **Review (2025-10-18, near midnight++):** Jurisdiction policy packs no longer
> depend on any serde helpers. The crate’s new `codec` module encodes
> `PolicyPack`/`SignedPack` through the first-party binary cursor, manual JSON
> diff builders replace the previous macro usage, and the refreshed regression
> suite rejects malformed payloads (missing fields, duplicates, unexpected keys,
> trailing bytes). FIRST_PARTY_ONLY runs now cover both JSON and binary
> snapshots via the same in-house paths, keeping sled-backed history in lockstep
> with the legacy JSON outputs.

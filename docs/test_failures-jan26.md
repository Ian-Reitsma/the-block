...
- **Protocol decision:** finality intentionally recalculates the supermajority threshold over the effective stake (total minus equivocations); the test now asserts that once 2/3 of that surviving stake coalesces the block finalizes, and the behaviour is documented in `docs/consensus_safety.md`. When finality pauses, inspect `telemetry.summary.finality` (or `/wrappers`) for `effective_total_stake`, `finality_threshold`, and `equivocated_stake` before touching the UNL or invoking governance. Reran `cargo test -p the_block --test pos_finality --all-features`.

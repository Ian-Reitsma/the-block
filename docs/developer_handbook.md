...
| **BlockTorch** | Deterministic tensor/autograd framework (blocktorch) for verified ML compute workloads | [`ECONOMIC_PHILOSOPHY_AND_GOVERNANCE_ANALYSIS.md`](ECONOMIC_PHILOSOPHY_AND_GOVERNANCE_ANALYSIS.md#part-xii-blocktorch--the-compute-framework-strategy) |
...
- `scripts/deploy-block_os-testnet.sh` spins up the Block OS energy stack (node + mock oracle + telemetry). Pair it with `docs/testnet/ENERGY_QUICKSTART.md` to exercise the `energy.*` RPCs locally.

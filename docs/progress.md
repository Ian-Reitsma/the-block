...
> **Review (2025-10-06):** Completed CLI parser migration across tooling/node/contract surfaces and refreshed dependency readiness summary.
...
CLI, node, light-client, and metrics-aggregator binaries now build exclusively on the runtime facade’s in-house backend; Tokio persists only as a dormant compatibility feature inside `crates/runtime` while the gateway HTTP stacks await their router swap.

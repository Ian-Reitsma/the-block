...
fn check_mode_reports_drift_and_writes_telemetry() {
    ...
    let summary_path = out_dir.join("dependency-check.summary.json");
    let summary_bytes = fs::read(&summary_path).expect("check summary written");
    let summary = json::value_from_slice(&summary_bytes).expect("summary parse");
    let status = summary
        .as_object()
        .and_then(|map| map.get("status"))
        .and_then(|value| value.as_str())
        .unwrap_or_default();
    assert_eq!(status, "drift", "summary encodes drift status");
    let counts = summary
        .as_object()
        .and_then(|map| map.get("counts"))
        .and_then(|value| value.as_object())
        .expect("counts map present");
    assert_eq!(
        counts.get("additions").and_then(|value| value.as_u64()),
        Some(1)
    );
    assert_eq!(
        counts
...
fn generates_all_artifacts_via_cli_runner() {
    ...
    if let Some(summary_path) = &artifacts.check_summary_path {
        assert!(
            summary_path.exists(),
            "summary path returned by runner exists"
        );
    }

...
pub fn execute(cli: &Cli) -> Result<RunArtifacts> {
    ...
    if cli.check {
        let baseline = match output::load_registry(&cli.baseline) {
            ...
            Err(err) => {
                let telemetry_result = output::write_check_telemetry(
                    &cli.out_dir,
                    &CheckTelemetry::baseline_error("load_failed"),
                );
        ...
        output::write_check_telemetry(&cli.out_dir, &CheckTelemetry::pass())?;
    }

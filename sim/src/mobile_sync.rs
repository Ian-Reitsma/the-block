...
pub fn measure_sync_latency(headers: Vec<Header>) -> std::time::Duration {
    ...
    let genesis = match iter.next() { Some(h) => h, None => return std::time::Duration::default() };
    let mut client = LightClient::new(genesis);
    let remaining: Vec<Header> = iter.collect();
    let fetch = move |start: u64| remaining.clone().into_iter().filter(|h| h.height >= start).collect();
    let opts = SyncOptions { wifi_only: false, require_charging: false, min_battery: 0.0 };

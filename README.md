...
- **BlockTorch Compute Framework**: The blocktorch stack (metal-tensor + autograd) provides deterministic tensor ops and gradient tracking for compute marketplace workloads, with CPU fallbacks, Metal kernels, and a PyTorch bridge for validation.
...
| **`blocktorch/`** | BlockTorch compute framework (metal-tensor + autograd) for deterministic tensor ops and Metal/CPU backends | FlashAttention bridge, profiling hooks, kernel/runtime stack, and migration path to verified compute workloads |

...
- **CohortKeyV2 rollout + reversible migrations** — Update `crates/ad_market/src/lib.rs`, `node/src/ad_policy_snapshot.rs`, `node/src/ad_readiness.rs`, and `docs/system_reference.md` so dual-write sled keys (`cohort_v1:*` + `cohort_v2:*`) can be replayed forward/back without losing legacy badge-only cohorts. Owners: `@ad-market`, `@gov-core`.
- **Presence attestation plumbing** — Extend `node/src/localnet`, `node/src/range_boost`, `node/src/service_badge.rs`, `node/src/read_receipt.rs`, and `crates/ad_market/src/attestation.rs` to mint, cache, and verify `PresenceReceipt` structs governed by `TB_PRESENCE_TTL_SECS`/`TB_PRESENCE_RADIUS_METERS`. Wire cancellations through `node/src/bin/node.rs` and expand `node/tests/ad_market_rpc.rs` to cover the new RPCs (`ad_market.list_presence_cohorts`, `ad_market.reserve_presence`). Owners: `@ad-market`, `@gateway-stack`.
...
- **Observability + dashboards** — Add the new counters/histograms (`ad_segment_ready_total`, `ad_auction_top_bid_usd_micros`, `ad_bid_shading_factor_bps`, `ad_privacy_budget_utilization_ratio`, `ad_conversion_value_ct_total`, etc.) to `metrics-aggregator/src/lib.rs` and refresh Grafana JSON/screenshots under `monitoring/ad_market_dashboard.json` per `docs/operations.md#telemetry-wiring`. Owners: `@telemetry-ops`, `@ad-market`.

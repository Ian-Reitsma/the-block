...
### 0.7 Licensing, CLA, and Trademark Guardrails
- **Open core stays permissive** — All chain-critical surfaces (consensus, networking/overlay, ledger/state, VM, receipts/validation, governance rules, protocol economics, node runtime/CLI/explorer/light clients/bridges) remain Apache 2.0 (or Apache/MIT where already used). No BSL/source-available/delayed-open licenses may land on validator/exchange paths.
- **Enterprise stack is proprietary** — Operational extras (managed validator orchestration, enterprise observability/incident tooling, compliance/reporting, hosted marketplace coordination/SLA services, premium SDKs/adapters/one-click deploys) live in a separate `the-block-enterprise` repo under a commercial EULA; optional BSL (18–36 month conversion) is allowed only for non-consensus enterprise modules.
- **CLA + DCO required** — Contributors must agree to [`CLA.md`](CLA.md) before merge (record via CLA portal/PR comment) and include `Signed-off-by:` trailers (`git commit -s`). CI (`scripts/check_cla.sh`, `node/tests/cla_check.rs`) enforces sign-off; maintainers keep the acceptance ledger tied to sign-off identities.
- **Trademark & canonical releases** — Forks must not use “The Block” name/logo without permission and should rebrand to avoid confusion. Only release artifacts signed by the published keys (see `config/release_signers.txt` and [`docs/security_and_privacy.md#release-provenance-and-supply-chain`](docs/security_and_privacy.md#release-provenance-and-supply-chain)) are “official.” Keep `docs/licensing_and_trademark.md` current with signing key fingerprints and policy changes.
- **Enterprise split published** — `the-block-enterprise` lives as a sibling repo (not a submodule) under a proprietary EULA. Keep consensus-critical code Apache-only in this repo; enterprise modules consume public APIs/telemetry/governance hooks only. Track compatibility and signing in the enterprise repo docs.
...
- **Chain sync reliability pass** — Validate the async gossip listener, periodic `ChainRequest` pull loop (`TB_P2P_CHAIN_SYNC_INTERVAL_MS`), and broadcast retry behavior in `node/src/net/peer.rs` + `node/src/net/mod.rs`; ensure `node/tests/net_integration.rs` + `node/tests/chaos.rs` converge on every run and keep `docs/architecture.md#networking-and-propagation` in sync with the final knobs/behavior.
...
### 15.M Licensing, CLA, and Official Chain Posture
- **Enterprise split + EULA** — Stand up the proprietary `the-block-enterprise` repo with a commercial EULA for operational extras (managed validator orchestration, enterprise observability/incident tooling, compliance/reporting, hosted marketplace coordination/SLA services, premium SDKs/adapters/one-click deploys) while keeping all consensus-critical code in this Apache-licensed core. Reference `docs/licensing_and_trademark.md` for scope boundaries. Owners: `@docs-core`, `@gov-core`.
- **CLA enforcement + ledger** — Enforce `CLA.md` acceptance before merge (PR comment/portal) and require `Signed-off-by:` trailers on every commit (`git commit -s`). Wire PR templates/CI to block merges lacking both CLA agreement and sign-off; maintain the contributor ledger tied to sign-off identities so relicensing remains clean. Owners: `@docs-core`, `@security`.
- **Trademark + signed-release policy** — Publish signing key fingerprints and the verification flow in `docs/security_and_privacy.md#release-provenance-and-supply-chain`, mirror the “official chain” rules from `docs/licensing_and_trademark.md` into operator runbooks, and ensure forks are instructed to rebrand when not authorized to use The Block marks. Owners: `@docs-core`, `@security`.

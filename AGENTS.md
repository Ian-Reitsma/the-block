...
**Latest highlights (2025-09-29 update: tightened readiness after confirming aggregator/gateway HTTP stacks still depend on hyper/axum):**
...
- Runtime-backed HTTP client coverage now spans the node/CLI stacks, but the metrics aggregator and gateway HTTP servers still ride on `axum` until the in-house router lands. Track the outstanding swaps in `docs/rpc.md` and `docs/roadmap.md`.

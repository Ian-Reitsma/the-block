...
- **Zero third-party rule** — All Rust/C++ dependencies must be first-party or fully vendored under our namespace; registry/git sources are forbidden. `FIRST_PARTY_ONLY=1` stays enabled, `dependency_guard` must pass, and reviewers reject PRs that add non-workspace crates. Update `config/dependency_policies.toml`, `provenance.json`, and `checksums.txt` whenever vendoring changes, and mirror the policy in the PR description plus `docs/operations.md`.

...
impl ReadBatcher {
    pub fn new() -> Self { Self { acks: Vec::new() } }
    ...
    pub fn finalize(&mut self) -> ReadBatch {
        ...
        if leaves.is_empty() {
            return ReadBatch { root: [0u8;32], total_bytes: 0, count: 0 };
        }
        while leaves.len() > 1 {
            ...
            for pair in leaves.chunks(2) {
                let mut h = Hasher::new();
                h.update(&pair[0]);
                if pair.len()==2 { h.update(&pair[1]); } else { h.update(&pair[0]); }
                next.push(h.finalize().into());
            }
            leaves = next;
        }
        self.acks.clear();
        ReadBatch { root: leaves[0], total_bytes, count }
    }
}

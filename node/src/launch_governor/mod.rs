...
impl OperationalController {
    ...
    fn evaluate(&mut self, epoch: u64, sample: &ChainSample) -> Option<PlannedEval> {
        ...
        let smooth_prev = *self
            .smooth_history
            .get(self.smooth_history.len() - 2)
            .unwrap();
        ...
        let replay_prev = *self
            .replay_history
            .get(self.replay_history.len() - 2)
            .unwrap();
...
impl NamingController {
    ...
    fn evaluate(&mut self, epoch: u64, sample: &DnsSample) -> Option<PlannedEval> {
        ...
        let dispute_prev = *self
            .dispute_history
            .get(self.dispute_history.len() - 2)
            .unwrap();
        let completion_prev = *self
            .completion_history
            .get(self.completion_history.len() - 2)
            .unwrap();
        let settlement_prev = *self
            .settlement_history
            .get(self.settlement_history.len() - 2)
            .unwrap();
        let duration_prev = *self
            .duration_history
            .get(self.duration_history.len() - 2)
            .unwrap();

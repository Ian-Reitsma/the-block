...
use the_block::net::{
    BlobChunk, Handshake, Message, Payload, PeerSet, REQUIRED_FEATURES, SUPPORTED_VERSION,
};
...
fn shard_rate_limiting() {
    ...
    peers.handle_message(Message::new(Payload::Handshake(hs), &sk), Some(addr), &chain);

    let chunk = BlobChunk { root: [1u8; 32], index: 0, total: 1, data: vec![0; 256] };
    peers.handle_message(Message::new(Payload::BlobChunk(chunk.clone()), &sk), Some(addr), &chain);
    let db = SimpleDb::open(dir.path().join("chunks").to_str().unwrap());
    assert_eq!(db.keys_with_prefix("chunk/").len(), 1);

    let chunk2 = BlobChunk { root: [1u8; 32], index: 1, total: 2, data: vec![0; 400] };
    peers.handle_message(Message::new(Payload::BlobChunk(chunk2), &sk), Some(addr), &chain);

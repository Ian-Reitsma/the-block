...
use log_indexer::{index_logs_with_options, IndexOptions};
...
fn search_filters_and_decryption() -> Result<()> {
    ...
    writeln!(file, "{{\"timestamp\":1,\"level\":\"INFO\",\"message\":\"ready\",\"correlation_id\":\"alpha\"}}")?;
    writeln!(file, "{{\"timestamp\":2,\"level\":\"ERROR\",\"message\":\"failed\",\"correlation_id\":\"beta\"}}")?;
    writeln!(file, "{{\"timestamp\":3,\"level\":\"WARN\",\"message\":\"retry\",\"correlation_id\":\"beta\"}}")?;
    ...
    let error_rows = the_block::rpc::logs::run_search_for_path(
        "/logs/search?level=ERROR&passphrase=secret",
    )?;
